...
import { getAIRecommendationsInsights } from '../services/geminiService';
import InputField from './InputField';
import { SparklesIcon, KeyIcon, UserGroupIcon, TagIcon, CurrencyDollarIcon } from '@heroicons/react/24/solid';
import { PlusIcon, TrashIcon, ClipboardDocumentIcon } from '@heroicons/react/24/outline';

/**
 * Component for the AI Recommendations tab.
 * Uses data from all tabs (product, cost, pricing) for comprehensive recommendations.
 */
const AIRecommendationsTab: React.FC = () => {
  ...
  /**
   * Handles changes to AI recommendations inputs.
   */
  const handleInputChange = (field: keyof eScalePricingInputs) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target;
    setEScalePricingInputs({
      ...eScalePricingInputs,
      [field]: field === 'targetProfitMargin' ? Number(value) : value
    });
  };

  /**
   * Handles strategy selection change.
   */
  const handleStrategyChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setEScalePricingInputs({
      ...eScalePricingInputs,
      selectedStrategy: e.target.value as eScalePricingInputs['selectedStrategy']
    });
  };
  ...
  const addCompetitor = () => {
    ...
    setEScalePricingInputs({
      ...eScalePricingInputs,
      competitors: [...eScalePricingInputs.competitors, newCompetitor]
    });
  };
   ...
   */
  const updateCompetitor = (id: string, field: keyof typeof eScalePricingInputs.competitors[0], value: any) => {
    setEScalePricingInputs({
      ...eScalePricingInputs,
      competitors: eScalePricingInputs.competitors.map(comp =>
        comp.id === id ? { ...comp, [field]: field === 'price' ? Number(value) : value } : comp
      )
    });
  };
  ...
  const removeCompetitor = (id: string) => {
    setEScalePricingInputs({
      ...eScalePricingInputs,
      competitors: eScalePricingInputs.competitors.filter(comp => comp.id !== id)
    });
  };
  ...
  const handleFetchInsights = useCallback(async () => {
    ...
    setInsights('');
    try {
      const response = await getAIRecommendationsInsights(
        productInputs,
        costInputs,
        calculatedCosts,
        eScalePricingInputs,
    ...
    setIsLoading(false);
  }, [productInputs, costInputs, calculatedCosts, eScalePricingInputs, currency, apiKey]);
  ...
  /**
   * Copies the insights text to clipboard.
   */
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-lg">
              <h2 className="text-xl font-bold text-slate-700 mb-4">معطيات التسعير</h2>

              {/* Competitors Section */}
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-slate-700">المنافسون</h3>
                  <button
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto">
                  {eScalePricingInputs.competitors.map((competitor) => (
                    <div key={competitor.id} className="p-3 bg-slate-50 rounded-lg border">
              ...
              <InputField
                ...
                onChange={handleInputChange('targetProfitMargin')}
              ...
              {/* Pricing Strategy */}
              <div>
                <label htmlFor="strategy" className="block text-sm font-medium text-slate-600 mb-2">
                  استراتيجية التسعير
                </label>
                <select
                  id="strategy"
                  value={eScalePricingInputs.selectedStrategy}
                ...
                >
                  ...
                  <option value="valueBased">قائم على القيمة</option>
                  <option value="marketPenetration">اختراق السوق</option>
                  <option value="priceSkimming">حلب السوق</option>
                </select>
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center space-x-3 space-x-reverse mb-4 lg:mb-0">
                  ...
                  <div>
                    ...
                    <p className="text-slate-gray mt-1 text-sm sm:text-base">احصل على توصيات تسعير وتحسين منتج مخصصة</p>
                  </div>
                </div>
                  ...
                  <div className="bg-gradient-to-r from-electric-teal/5 to-bright-cyan/5 p-6 rounded-xl border border-electric-teal/10">
                    ...
                    <div className="mt-6 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 bg-electric-teal text-white font-medium rounded-lg hover:bg-electric-teal/90 focus-ring transition-all duration-200"
                      >
